...
KUBEVIRT_VM_NAMESPACE = "kubevirt-vms"
...
class KubevirtSetupKeywords(BaseKeyword):
    ...
    def ensure_kubevirt_ready(self, namespace: str = KUBEVIRT_VM_NAMESPACE) -> None:
        """Ensure KubeVirt is applied and a test VM instance is Running. Idempotent.

        Applies the kubevirt-app (no-op if already applied), creates the target
        namespace if it does not exist, then deploys the test VM into that namespace
        and waits for its VMI to reach Running. If the VM is already Running in the
        target namespace this returns immediately, so it is safe to call at the start
        of every test that needs a running VMI without depending on test order or the
        lab being pre-provisioned.

        Args:
            namespace (str): Namespace to deploy the VM into. Created if it does not
                already exist. Defaults to 'kubevirt-vms'.
        """
        ...
        KubectlCreateNamespacesKeywords(self.ssh_connection).create_namespace_if_absent(namespace)

        vmi_keywords = KubectlGetVmiKeywords(self.ssh_connection)
        if vmi_keywords.is_vmi_present(KUBEVIRT_VM_NAME, namespace):
            vmi_status = vmi_keywords.get_vmi_status(KUBEVIRT_VM_NAME, namespace)
        else:
            vmi_status = ""

        if vmi_status == "Running":
            get_logger().log_info(f"KubeVirt VM '{KUBEVIRT_VM_NAME}' already Running in namespace '{namespace}' - skipping deploy")
            return

        get_logger().log_info(f"Deploying KubeVirt VM '{KUBEVIRT_VM_NAME}' in namespace '{namespace}'")
        ...
        # after the app is applied, so retry the apply until it succeeds. The VM
        # manifest has no metadata.namespace, so '-n <namespace>' lands it in the
        # target namespace.
        KubectlFileApplyKeywords(self.ssh_connection).apply_resource_from_yaml_with_retry(KUBEVIRT_REMOTE_VM_YAML, namespace=namespace, timeout=KUBEVIRT_APPLY_RETRY_TIMEOUT_SECONDS)

        get_logger().log_info(f"Waiting for KubeVirt VM '{KUBEVIRT_VM_NAME}' to be Running")
        KubectlGetVmKeywords(self.ssh_connection).wait_for_vm_status(KUBEVIRT_VM_NAME, "Running", namespace, timeout=KUBEVIRT_VM_READY_TIMEOUT_SECONDS)
        vmi_keywords.wait_for_vmi_status(KUBEVIRT_VM_NAME, "Running", namespace, timeout=KUBEVIRT_VM_READY_TIMEOUT_SECONDS)
