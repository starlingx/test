...
class TS2PHCSetup:
    ...
    def __init__(self, setup_dict: Dict[str, Any], ptp_host_ifs_dict: Dict[str, PTPHostInterfaceSetup]):
        ...
        if "instance_hostnames" not in setup_dict:
            raise Exception(f"The ts2phc entry {self.name} must have instance_hostnames defined.")
        self.instance_hostnames = setup_dict["instance_hostnames"]

        if "instance_parameters" not in setup_dict:
            raise Exception(f"The ts2phc entry {self.name} must have instance_parameters defined.")
        self.instance_parameters = setup_dict["instance_parameters"]

        if "ptp_interface_names" not in setup_dict:
            raise Exception(f"The ts2phc entry {self.name} must have ptp_interface_names defined.")
    ...
    def get_name(self) -> str:
        """
        Gets the name of this ts2phc setup.

        Returns:
            str: The name of this ts2phc setup.
        """
        return self.name
