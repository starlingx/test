...
METRIC_REPORTER_CONFIG_TEMPLATE = {"cluster": {"host": "{host_url}", "distributedcloud": "no", "metric_url": "/mon-elasticsearch-client/metricbeat-*/_search?scroll=", "scroll_url": "/mon-elasticsearch-client/_search/scroll", "username": "{target_system_user}", "password": "{target_system_password}"}, "dc": {"ip_addr": "not a dc system", "ssh_port": "", "ssh_pass": "not a dc system"}, "filters": {"__SYSNAME__": "{lab_name}", "__START_TS__": "{start_time}", "__END_TS__": "{end_time}", "__TIMEZONE__": "UTC", "__QSIZE__": "4096", "__SCROLL__": "15m"}, "datasets": [{"name": "system.core", "query": "queries/scrolling_dataset.json", "csv": "true"}, {"name": "system.cpu", "query": "queries/scrolling_dataset.json", "csv": "true"}, {"name": "linux.iostat", "query": "queries/scrolling_dataset.json", "csv": "true"}, {"name": "system.diskio", "query": "queries/scrolling_dataset.json", "csv": "true"}, {"name": "system.filesystem", "query": "queries/scrolling_dataset.json", "csv": "true", "exclude": ["^/var/lib/kubelet/plugins/kubernetes.io/rbd/mounts"]}, {"name": "system.load", "query": "queries/scrolling_dataset.json", "csv": "true"}, {"name": "system.memory", "query": "queries/scrolling_dataset.json", "csv": "true"}, {"name": "system.network", "query": "queries/scrolling_dataset.json", "csv": "true", "exclude": ["^cali"]}, {"name": "system.process", "query": "queries/scrolling_dataset.json", "csv": "true", "period": "10"}, {"name": "system.top_n", "query": "queries/scrolling_dataset.json", "csv": "true", "top_n": "", "hosts": ""}, {"name": "kubernetes.container", "query": "queries/scrolling_dataset.json", "csv": "true", "period": "10"}, {"name": "kubernetes.pod", "query": "queries/scrolling_dataset.json", "csv": "true", "period": "10"}, {"name": "kubernetes.node", "query": "queries/scrolling_dataset.json", "csv": "true", "period": "10"}], "reporting": {"output_dir": "{result_output_dir}", "json": "true", "tabular": "true"}}
...
class MetricReporterKeywords(BaseKeyword):
    ...
    def run_metric_reporter(self, start_time: datetime, end_time: datetime) -> str:
        ...
        """
        get_logger().log_info("Loading metric server configuration...")
    ...
    def _create_config_content(self, start_time: datetime, end_time: datetime, result_output_dir: str, server_config: dict) -> str:
        """
        ...
        sys_controller_ip = lab_config.get_system_controller_ip()
        if sys_controller_ip is not None:
            lab_ip = sys_controller_ip
