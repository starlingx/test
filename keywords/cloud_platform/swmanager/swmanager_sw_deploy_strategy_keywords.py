...
class SwManagerSwDeployStrategyKeywords(BaseKeyword):
            ...
            try:
                ...
                if "connection" in error_msg or "timeout" in error_msg or "nonetype" in error_msg:
                    ...
                    if connection_retry_count <= max_connection_retries:
                        ...
                        get_logger().log_warning("Max connection retries reached, continuing with longer interval...")
                        connection_retry_count = 0
                        time.sleep(30)
                else:
    ...
    def check_sw_deploy_strategy_exists(self) -> bool:
        ...
        """
        try:
            self.get_sw_deploy_strategy_show()
            return True
        except Exception:
            return False
    ...
    def wait_for_state(self, state: list, timeout: int = 1800) -> bool:
        ...
        Returns:
            bool: True if desired state is reached, False if timeout or terminal failure.
        """
        ...
        while time.time() < end_time:
            try:
                ...
                current_state = output.get_state()

                if current_state in state:
                    return True

                if current_state in terminal_failure_states:
                    get_logger().log_error(f"Strategy reached terminal failure state: {current_state}")
                    return False
                ...
                if "connection" in error_msg or "timeout" in error_msg:
                    if connection_issues <= 5:
                        ...
                        get_logger().log_warning("Extended connection issues detected, continuing to monitor...")
                else:
