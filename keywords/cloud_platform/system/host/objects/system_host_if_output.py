...
class SystemHostInterfaceOutput:
    ...
    def __init__(self, system_host_interface_output: str):
        """
        Constructor

        Args:
            system_host_interface_output (str): String output of 'system host-if-list' command
    ...
    def has_ae_interface(self) -> bool:
        """Check for at least one Aggregated Ethernet (AE) network interface type.
        ...
        Returns:
            bool: True if there is at least one Aggregated Ethernet (AE) network interface type in the list of interfaces,
                and False otherwise.
        """
        return any(item.get_type() == "ae" for item in self.system_host_interfaces)

    def has_bond_interface(self) -> bool:
        """Check for at least one Bond network interface.
        ...
        Returns:
            bool: True if there is at least one Bond network interface in the list of interfaces, and False otherwise.
        """
    ...
    def get_physical_interface_count(self) -> int:
        """Count the number of physical interfaces on this host.
        ...
        Returns:
            int: The number of physical interfaces on this host.
        """
    ...
    def get_interfaces_by_class(self, if_class: str) -> list[SystemHostInterfaceObject]:
        """Get all interfaces with the given class.

        Args:
            if_class (str): The interface class to filter by.
