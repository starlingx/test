...
class PTPScenarioExecutorKeywords(BaseKeyword):
    ...
    def __init__(self, ssh_connection, resource_path: str):
        ...
        self.interfaces_to_cleanup = []
        self.gnss_to_cleanup = []
        self.sma_to_cleanup = []
        self.services_to_cleanup = []
    ...
    def execute_test_scenario(self, test_scenario_data, request=None) -> None:
        ...
        def cleanup_operations():
            """Revert all operations to original state."""
            if self.interfaces_to_cleanup:
                get_logger().log_info("Cleanup: Reverting interface operations")
                ip_keywords = IPKeywords(self.ssh_connection)
                for interface_name in self.interfaces_to_cleanup:
                    get_logger().log_info(f"Bringing up interface {interface_name}")
                    ip_keywords.set_ip_port_state(interface_name, StatusConstants.interface_up)

            if self.gnss_to_cleanup:
                get_logger().log_info("Cleanup: Reverting GNSS operations")
                for hostname, nic in self.gnss_to_cleanup:
                    get_logger().log_info(f"Turning GNSS back on for {hostname} {nic}")
                    self.gnss_keywords.gnss_power_on(hostname, nic)

            if self.sma_to_cleanup:
                get_logger().log_info("Cleanup: Reverting SMA operations")
                for hostname, nic in self.sma_to_cleanup:
                    get_logger().log_info(f"Enabling SMA for {hostname} {nic}")
                    self.sma_keywords.enable_sma(hostname, nic)

            if self.services_to_cleanup:
                get_logger().log_info("Cleanup: Reverting service operations")
                for service_name, instance_name in self.services_to_cleanup:
                    get_logger().log_info(f"Starting {service_name}@{instance_name}.service")
                    self.systemctl_keywords.systemctl_start(service_name, instance_name)

        if request:
            request.addfinalizer(cleanup_operations)
        ...
        for step in steps:
            ...
            self.execute_operations(operations, request, verification)
            self.execute_verification(verification)

    def execute_operations(self, operations: list, request=None, extra_args: list = None) -> None:
        """Execute operations based on type.

        Args:
            operations (list): List of operation dictionaries.
            request: Pytest request fixture for finalizers (optional).
            extra_args (list): List with additional data required by operation (optional).
        """
        for operation in operations:
            ...
            if operation_type == OperationType.interface:
                ...
                self._execute_phc_ctl_loop_operation(operation, extra_args)
            elif operation_type == OperationType.proxmox:
                self._execute_proxmox_operation(operation, request, extra_args)
            else:
    ...
    def _execute_interface_operation(self, operation: dict) -> None:
        ...
        interface_name = nic_obj.get_nic_connection().get_interface()

        ip_keywords = IPKeywords(self.ssh_connection)

        if status == StatusConstants.interface_down:
            get_logger().log_info(f"Interface down {interface_name}.")
            ip_keywords.set_ip_port_state(interface_name, StatusConstants.interface_down)
            if interface_name not in self.interfaces_to_cleanup:
                self.interfaces_to_cleanup.append(interface_name)
        else:
            get_logger().log_info(f"Interface up {interface_name}.")
            ip_keywords.set_ip_port_state(interface_name, StatusConstants.interface_up)
            if interface_name in self.interfaces_to_cleanup:
                self.interfaces_to_cleanup.remove(interface_name)
    ...
    def _execute_gnss_operation(self, operation: dict) -> None:
        ...
        """
        status = operation.get("status")
        gnss_config = operation.get("gnss_config", {})
        hostname = gnss_config.get("hostname")
        nic = gnss_config.get("nic")
        gnss_key = (hostname, nic)

        if status == StatusConstants.gnss_sma_disable:
            get_logger().log_info(f"Turning off GNSS for {hostname} {nic}.")
            self.gnss_keywords.gnss_power_off(hostname, nic)
            if gnss_key not in self.gnss_to_cleanup:
                self.gnss_to_cleanup.append(gnss_key)
        else:
            get_logger().log_info(f"Turning GNSS back on for {hostname} {nic}.")
            self.gnss_keywords.gnss_power_on(hostname, nic)
            if gnss_key in self.gnss_to_cleanup:
                self.gnss_to_cleanup.remove(gnss_key)
    ...
    def _execute_sma_operation(self, operation: dict) -> None:
        ...
        """
        status = operation.get("status")
        sma_config = operation.get("sma_config", {})
        hostname = sma_config.get("hostname")
        nic = sma_config.get("nic")
        sma_key = (hostname, nic)

        if status == StatusConstants.gnss_sma_disable:
            get_logger().log_info(f"Disabled SMA1 for {hostname} {nic}.")
            self.sma_keywords.disable_sma(hostname, nic)
            if sma_key not in self.sma_to_cleanup:
                self.sma_to_cleanup.append(sma_key)
        else:
            get_logger().log_info(f"Enabled SMA1 for {hostname} {nic}.")
            self.sma_keywords.enable_sma(hostname, nic)
            if sma_key in self.sma_to_cleanup:
                self.sma_to_cleanup.remove(sma_key)
    ...
    def _execute_service_operation(self, operation: dict) -> None:
        ...
        """
        status = operation.get("status")
        service_config = operation.get("service_config", {})
        service_name = service_config.get("service_name")
        instance_name = service_config.get("instance_name")
        service_key = (service_name, instance_name)

        if status == StatusConstants.service_stop:
            get_logger().log_info(f"Stopping {service_name}@{instance_name}.service...")
            self.systemctl_keywords.systemctl_stop(service_name, instance_name)
            time.sleep(10)
            if service_key not in self.services_to_cleanup:
                self.services_to_cleanup.append(service_key)
        elif status == StatusConstants.service_start:
            get_logger().log_info(f"Starting {service_name}@{instance_name}.service...")
            self.systemctl_keywords.systemctl_start(service_name, instance_name)
            if service_key in self.services_to_cleanup:
                self.services_to_cleanup.remove(service_key)
        elif status == StatusConstants.service_restart:
            get_logger().log_info(f"Restarting {service_name}@{instance_name}.service...")
            self.systemctl_keywords.systemctl_restart(service_name, instance_name)
        else:
    ...
    def _resolve_interface_placeholder(self, entity_id: str) -> str:
        ...
        Args:
            entity_id (str): Entity ID with potential {{ ... }} placeholder.
        ...
        match = re.search(r"{{\s*(\w+)\.(nic\d+)\.([\w.]+)\s*}}", entity_id)
        if not match:
            return entity_id

        ptp_config = ConfigurationManager.get_ptp_config()
        host = ptp_config.get_host(match.group(1))
        nic = host.get_nic(match.group(2))
        path = match.group(3)

        if path == "base_port":
            interface = nic.get_base_port()
        elif path == "nic_connection.interface":
            interface = nic.get_nic_connection().get_interface()
        elif path == "conn_to_proxmox":
            interface = nic.get_conn_to_proxmox()
        else:
            return entity_id

        return entity_id.replace(match.group(0), interface)
    ...
    def _verify_alarms(self, verify_step: dict) -> None:
        ...
        state = alarm_dicts[0].get("state", StatusConstants.alarm_set)
        if state == StatusConstants.alarm_set:
    ...
    def _verify_pmc(self, verify_step: dict) -> None:
        ...
        check_domain = not (self.last_operation and self.last_operation.get("type") == OperationType.service and self.last_operation.get("status") == StatusConstants.service_stop)
    ...
    def _execute_phc_ctl_operation(self, operation: dict) -> None:
        ...
        if status == StatusConstants.phc_ctl_adj:
            phc_ctl_keywords.phc_ctl_adj(interface_name, "0.0001")
        elif status == StatusConstants.phc_ctl_set:
            phc_ctl_keywords.phc_ctl_set(interface_name)
        else:
            get_logger().log_info(f"Invalid status has been selected {status}")

    def _execute_phc_ctl_loop_operation(self, operation: dict, extra_args: list = None) -> None:
        """Execute PHC control loop operation that waits for alarms.

        Args:
            operation (dict): Operation configuration dictionary.
            extra_args (list): List of additional arguments (optional).
                               For phc_ctl loop, expects some alarms info
                               that are used as exit condition.
        """
        interface_mapping = operation.get("interface_mapping", {})
        hostname = interface_mapping.get("hostname")
        nic = interface_mapping.get("nic")
        ...
        alarm_objects = []
        if extra_args:
            for verify_step in extra_args:
                if verify_step.get("type") == VerificationType.alarm:
                    alarm_dicts = verify_step.get("alarms", [])
                    for alarm_dict in alarm_dicts:
                        alarm_obj = AlarmListObject()
                        alarm_obj.set_alarm_id(alarm_dict.get("alarm_id", ""))
    ...
    def _execute_proxmox_operation(self, operation: dict, request=None, extra_args: list = None) -> None:
        ...
        if extra_args:
            for verify_step in extra_args:
                if verify_step.get("type") == VerificationType.pmc_value:
                    pmc_values = verify_step.get("pmc_values", [])
                    break

        ptp_instance_selection = [[pmc_val["name"], hostname, []] for pmc_val in pmc_values for hostname in pmc_val.keys() if hostname != "name"]
        ptp_expected_setup = self.ptp_setup_keywords.filter_and_render_ptp_config(self.resource_path, ptp_instance_selection)
        ...
        proxmox_verification_keywords.validate_port_data_set_with_retry(pmc_keywords, proxmox_config, ptp_config_file, 300)
        proxmox_verification_keywords.validate_parent_data_set_with_retry(pmc_keywords, ptp1_expected_config, proxmox_config, ptp_config_file, 300)
        proxmox_verification_keywords.validate_parent_port_identity_with_retry(pmc_keywords, proxmox_config, ptp_config_file, ssh_connection, 300)
        proxmox_verification_keywords.validate_time_properties_data_set_with_retry(pmc_keywords, ptp1_expected_config, proxmox_config, ptp_config_file, 300)
