...
@mark.p2
def test_pre_upgrade_check_metrics_server_app():
    """
    Check ceph health and ensure the metrics-server application is applied.

    Test Steps:
        - Verify ceph health status is healthy
        - Check if metrics-server application is present and applied
        - Upload and apply metrics-server if not present
        - Apply metrics-server if present but not in applied state
    """
    ...
    if not app_list_keywords.is_app_present(metrics_server_name):
        get_logger().log_test_case_step("Upload metrics-server app.")
        ...
        get_logger().log_test_case_step("Apply metrics-server app.")
...
@mark.p2
def test_post_upgrade_check_metrics_server_app():
    """
    Verify ceph health and metrics-server application status after upgrade, then remove, apply, abort, and re-apply.

    Test Steps:
        - Verify ceph health status is healthy
        - Validate the metrics-server application is present and in applied state
        - Remove the metrics-server application using "system application-remove"
        - Apply the metrics-server application without waiting for completion
        - Abort the metrics-server application using "system application-abort"
        - Validate the application status changed to apply-failed
        - Apply the metrics-server application again to ensure it can be applied after aborting
        - Verify ceph health status is healthy
    """
    active_ssh_connection = LabConnectionKeywords().get_active_controller_ssh()
    metrics_server_name = ConfigurationManager.get_app_config().get_metric_server_app_name()

    get_logger().log_test_case_step("Checking ceph health.")
    CephStatusKeywords(active_ssh_connection).wait_for_ceph_health_status(expect_health_status=True)

    get_logger().log_test_case_step("Validate metrics-server app is applied.")
