...
@mark.lab_has_dsa
@mark.lab_is_simplex
def test_dsa_accel_config_pod_shared_queues_only_simplex(request):
    """Test case to install and validate Intel DSA plugin with shared queues only on simplex lab.
    ...
    **Test Steps**
    ...
    4. Apply IDP and verify the chart is enabled.
    5. Verify DSA queues configuration (simplex default is shared queues only).
    6. Create default-registry-key secret in default namespace.
    7. Ensure stx-debian-tools-dev:stx.10.0-v1.0.0 is available on registry.
    8. Upload and apply the DSA accel-config test pod YAML.
    9. Wait for the pod to reach Completed status and validate logs success markers.
    """
    ...
    delete_kw = SystemApplicationDeleteKeywords(ssh_connection)
    ...
    # 3) Verify DSA queues configuration (simplex - default is shared queues)
    get_logger().log_test_case_step("Verify DSA queues configuration (simplex - default is shared queues)")
    ...
    get_logger().log_info(f"Node {node.get_name()} allocatable resources: {node.get_allocatable()}")
    get_logger().log_info("Simplex lab detected. Default configuration already uses shared queues only.")

    # 4) Create default-registry-key secret in default (copy from kube-system)
    ...
    # 5) Ensure stx-debian-tools-dev:stx.10.0-v1.0.0 is available on registry
    ...
    # 6) Upload dsa-pod-shared.yaml and apply it
    get_logger().log_test_case_step("Upload dsa-pod-shared.yaml and apply it")
    repo_pod_yaml = "resources/cloud_platform/accel_config_pods/dsa-pod-shared.yaml"
    remote_pod_yaml = "/home/sysadmin/dsa-pod-shared.yaml"
    ...
    # 7) Wait up to 40 seconds for pod to reach Completed status
    ...
    # 8) Validate accel-config pod logs
