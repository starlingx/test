...
@mark.lab_has_dsa
@mark.lab_is_duplex
def test_dsa_accel_config_pod_shared_queues_only_duplex(request):
    """Test  case to install and validate Intel DSA plugin with shared queues only on duplex lab.
    ...
    **Test Steps**
    1. Ensure Node Feature Discovery (NFD) is uploaded and applied.
    2. Upload Intel Device Plugins application (IDP) and enable the DSA Helm chart.
    3. Apply IDP and verify the chart is enabled.
    4. If necessary, apply the shared-queues-only override and re-apply the IDP.
    5. Verify allocatable resources reflect only shared queues (no dedicated queues).
    6. Ensure default-registry-key secret exists in the `default` namespace.
    7. Upload and apply the DSA accel-config test pod YAML.
    8. Wait for the pod to reach `Completed` status and validate logs success markers.
    """
    ...
    repo_override_path = "resources/cloud_platform/accel_config_pods/dsa-override-shared.yaml"
    remote_override_path = "/home/sysadmin/dsa-override-shared.yaml"
    ...
    # 4) Apply shared-only override and re-apply IDP
    get_logger().log_test_case_step("Apply shared-only override and re-apply IDP")
    helm_override_kw.update_helm_override(
        "/home/sysadmin/dsa-override-shared.yaml",
    ...
    validate_equals(dedicated_total, 0, "Only shared queues remain after override")
    ...
    # 7) Upload dsa-pod-shared.yaml and apply it
