...
@mark.lab_has_dsa
@mark.lab_is_duplex
def test_dsa_accel_config_pod_shared_and_dedicated_queues_duplex(request):
    """Test case to install and validate Intel DSA plugin with shared and dedicated queues on duplex lab.
    ...
    **Test Steps**
    ...
    4. Configure DSA with shared and dedicated queues using helm override.
    5. Apply IDP and verify the chart is enabled.
    6. Verify DSA queues configuration (both shared and dedicated queues on both nodes).
    ...
    9. Upload and apply the DSA accel-config test pod YAML (shared config).
    10. Wait for the shared pod to reach Completed status and validate logs success markers.
    11. Delete shared pod and upload the dedicated pod YAML.
    12. Wait for the dedicated pod to reach Completed status and validate logs success markers.
    """
    ...
    repo_override_path = "resources/cloud_platform/accel_config_pods/dsa-override-shared-dedicated.yaml"
    remote_override_path = "/home/sysadmin/dsa-override-shared-dedicated.yaml"
    ...
    get_logger().log_info("=== DSA Queue Distribution BEFORE Override ===")
    shared_total_before = nodes_json.count_resource_total("dsa.intel.com/wq-user-shared")
    dedicated_total_before = nodes_json.count_resource_total("dsa.intel.com/wq-user-dedicated")
    for node in nodes_json.get_node_objects():
        node_shared = node.get_allocatable().get("dsa.intel.com/wq-user-shared", "0")
        node_dedicated = node.get_allocatable().get("dsa.intel.com/wq-user-dedicated", "0")
        get_logger().log_info(f"{node.get_name()}: shared={node_shared}, dedicated={node_dedicated}")
    get_logger().log_info(f"Total BEFORE override: shared={shared_total_before}, dedicated={dedicated_total_before}")

    # 4) Apply shared-dedicated override and re-apply IDP
    get_logger().log_test_case_step("Apply shared-dedicated override and re-apply IDP")
    helm_override_kw.update_helm_override(
        "/home/sysadmin/dsa-override-shared-dedicated.yaml",
    ...
    apply_kw_mixed = SystemApplicationApplyKeywords(ssh_connection)
    apply_kw_mixed.system_application_apply(intel_plugins_app_name)
    ...
    validate_not_equals(shared_total, 0, "Shared queues available after override on both nodes")

    dedicated_total = nodes_json_after.count_resource_total("dsa.intel.com/wq-user-dedicated")
    validate_not_equals(dedicated_total, 0, "Dedicated queues available after override on both nodes")
    get_logger().log_info("=== DSA Queue Distribution Summary ===")
    for node_after in nodes_json_after.get_node_objects():
        node_shared = node_after.get_allocatable().get("dsa.intel.com/wq-user-shared", "0")
        node_dedicated = node_after.get_allocatable().get("dsa.intel.com/wq-user-dedicated", "0")
        get_logger().log_info(f"{node_after.get_name()}: shared={node_shared}, dedicated={node_dedicated}")

    get_logger().log_info(f"Total across all nodes: shared={shared_total}, dedicated={dedicated_total}")
    ...
    # Test both shared and dedicated pods sequentially.
    # 7) Test shared queues with dsa-pod-shared.yaml first
    ...
    # 8) Test dedicated queues with dsa-pod-dedicated.yaml
