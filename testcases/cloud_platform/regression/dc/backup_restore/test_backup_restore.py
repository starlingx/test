...
def test_restore_remote_backup_prestaged_subcloud_inactive_load(request):
    ...
    central_ssh = LabConnectionKeywords().get_active_controller_ssh()
    subcloud_release = CloudPlatformVersionManagerClass().get_last_major_release()
    get_logger().log_info(f"Subcloud release {subcloud_release}")
    ...
    obj_subcloud = dcmanager_subcloud_list_keywords.get_dcmanager_subcloud_list().get_subcloud_by_name(subcloud_name)
    # Verify that the prestage is completed successfully
    validate_equals(obj_subcloud.get_prestage_status(), "complete", f"subcloud {subcloud_name} successfully.")
    ...
    # Create a subcloud backup on central
    get_logger().log_info(f"Create {subcloud_name} backup on central")
    dc_manager_backup.create_subcloud_backup(subcloud_password, central_ssh, path=f"{central_path}/{subcloud_name}/{subcloud_release}", subcloud=subcloud_name)
    ...
    DcManagerSubcloudManagerKeywords(central_ssh).get_dcmanager_subcloud_unmanage(subcloud_name, 10)
    # Restore subcloud remote backup
    dc_manager_backup.restore_subcloud_backup(subcloud_password, central_ssh, subcloud=subcloud_name, release=subcloud_release, with_install=True)
