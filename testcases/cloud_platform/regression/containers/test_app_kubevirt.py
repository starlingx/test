...
def test_launch_debian12_vm_with_cdi_upload(request: FixtureRequest):
    ...
    This test validates the complete workflow of deploying a Debian 12 (bookworm)
    virtual machine using KubeVirt with CDI (Containerized Data Importer) for
    image management. It downloads a Debian 12 bookworm QCOW2 cloud image,
    uploads it to persistent storage via virtctl image-upload, and launches
    a VM with cloud-init.
    ...
    Test Steps:
        - Setup kubevirt environment
        - Copy VM yaml file to controller
        - Download Debian 12 bookworm QCOW2 image
        - Upload image to CDI using virtctl image-upload
        - Apply VM manifest with cloud-init configuration
        - Verify VM reaches Running state within 180 seconds
        - Verify VMI is ready with IP assignment
    ...
    ssh_connection = LabConnectionKeywords().get_active_controller_ssh()
    vm_name = "debian1280"
    pvc_name = "debian1280qcow"
    pvc_size = "5Gi"
    image_name = "debian-12-generic"
    vm_manifest = "vm-debian12.yaml"
    remote_yaml_path = f"{KUBEVIRT_VM_DIR}/{vm_manifest}"
    ...
    get_logger().log_test_case_step("Download Debian 12 bookworm QCOW2 image")
    kubevirt_images.download_image(image_name, KUBEVIRT_VM_DIR)
...
def test_launch_ubuntu_lunar_vm_with_cdi_upload(request: FixtureRequest) -> None:
    ...
    This test validates the complete workflow of deploying an Ubuntu 23.04 (Lunar)
    virtual machine using KubeVirt with CDI (Containerized Data Importer) for
    image management. It downloads an Ubuntu 23.04 Lunar cloud image, uploads it
    to persistent storage via virtctl image-upload, and launches a VM with cloud-init.
    ...
    Test Steps:
        - Setup kubevirt environment
        - Copy VM yaml file to controller
        - Download Ubuntu 23.04 Lunar cloud image
        - Create DataVolume and wait for upload ready
        - Upload image to CDI using virtctl image-upload
        - Apply VM manifest with cloud-init configuration
        - Verify VM reaches Running state within 180 seconds
        - Verify VMI is ready with IP assignment
    ...
    ssh_connection = LabConnectionKeywords().get_active_controller_ssh()
    vm_name = "ubuntu2304"
    pvc_name = "ubuntu2304img"
    pvc_size = "5Gi"
    image_name = "ubuntu-23.04-lunar"
    vm_manifest = "vm-ubuntu.yaml"
    remote_yaml_path = f"{KUBEVIRT_VM_DIR}/{vm_manifest}"
    ...
    get_logger().log_test_case_step("Download Ubuntu 23.04 Lunar cloud image")
    kubevirt_images.download_image(image_name, KUBEVIRT_VM_DIR)
...
def test_launch_vm_with_configured_memory_512m(request: FixtureRequest) -> None:
    ...
    This test validates deploying a virtual machine with specific memory configuration
    using KubeVirt with containerDisk image. It demonstrates launching a Cirros VM with
    512MB of memory using a pre-built container disk image, then verifies the VM is
    running and the memory allocation is correct.
