...
def get_vrb2_devices_address(ssh_connection: SSHConnection) -> list[tuple[str, str]]:
    """Get VRB2 devices address for all hosts in the system.

    Args:
        ssh_connection (SSHConnection): SSH connection to the active controller.

    Returns:
        list[tuple[str, str]]: Tuples of hostname and VRB2 device address.
    """
    ...
    for host_name in system_host_list_output.get_host_names():
        ...
        for device in device_output.get_device_address_by_device_id(VRB2_DEVICE_ID):
            vrb2_devices.append((host_name, device))

    for host_name, address in vrb2_devices:
        get_logger().log_info(f"Host: {host_name}, Address: {address}")

    return vrb2_devices


def generate_vrb2_configuration_file(ssh_connection: SSHConnection, vrb2_devices: list[tuple[str, str]], pf_driver: str, vf_driver: str, base_path: str) -> None:
    """Generate VRB2 configuration file and pod YAML file from template.

    Args:
        ssh_connection (SSHConnection): SSH connection to the target host.
        vrb2_devices (list[tuple[str, str]]): Tuples of hostname and VRB2 device address.
    ...
    pod_config_files = []
    for index, (host_name, device_adress) in enumerate(vrb2_devices):
        template_path = get_stx_resource_path("resources/cloud_platform/sriov-fec-operator/sriov-fec-operator-vrb2.yaml.j2")
        replacement_dict = {
            "name": f"vrb2-{index}",
            "hostName": host_name,
            "pfDriver": pf_driver,
        ...
        remote_file = yaml_kw.generate_yaml_file_from_template(template_path, replacement_dict, f"sriov-fec-operator-{pf_driver}-{vf_driver}-vrb2-{index}.yaml", base_path, preserve_order=True)
        operator_config_files.append(remote_file)
        get_logger().log_info(f"Generated operator config: {remote_file}")

        template_path = get_stx_resource_path("resources/cloud_platform/sriov-fec-operator/sriov-pod-vrb2.yaml.j2")
        replacement_dict = {
            "podName": f"vrb2-{index}",
        }
        remote_file = yaml_kw.generate_yaml_file_from_template(template_path, replacement_dict, f"sriov-pod-k8s-manifest-{pf_driver}-{vf_driver}-vrb2-{index}.yaml", base_path, preserve_order=True)
...
def apply_vrb2_configuration_and_pod(ssh_connection: SSHConnection, pf_driver: str, vf_driver: str, index: int, base_path: str, host_name: str) -> str:
    """Apply sriov-fec-operator VRB2 configuration and run accelerator pod and verify if pod is running.
    ...
    pod_file = f"{base_path}/sriov-pod-k8s-manifest-{pf_driver}-{vf_driver}-vrb2-{index}.yaml"
    ...
    get_logger().log_info("Verify if sriov-fec-operator VRB2 card configuration file was applied and pod is running")
    ...
    KubectlGetPodsKeywords(ssh_connection).wait_for_pods_to_reach_status(expected_status="Running", namespace="sriov-fec-system")
    get_logger().log_info("Run VRB2 pod for accelerator card")
    ...
    get_logger().log_info("Verify VRB2 pod is running")
    pod_name = get_pods_kw.get_pods(namespace="default").get_unique_pod_matching_prefix(starts_with="vrb2")
    pod_status = get_pods_kw.wait_for_pod_status(pod_name, "Running", "default")
    validate_equals(pod_status, True, "VRB2 pod status is running")
...
def delete_vrb2_configuration_and_pod(ssh_connection: SSHConnection, base_path: str, pf_driver: str, vf_driver: str, index: int, host_name: str) -> None:
    """Delete VRB2 operator configuration and pod resource files.
    ...
    kubectl_delete_kw.delete_resources(f"{base_path}/sriov-fec-operator-{pf_driver}-{vf_driver}-vrb2-{index}.yaml")
    get_logger().log_info("Verify if sriov-fec-operator VRB2 card configuration file was applied and pod is running")
    KubectlGetSriovVrbNodeConfigKeywords(ssh_connection).wait_for_configured_status(
        host_name,
        expected_status="InProgress",
    ...
    KubectlGetSriovVrbNodeConfigKeywords(ssh_connection).wait_for_configured_status(host_name)
    KubectlGetPodsKeywords(ssh_connection).wait_for_pods_to_reach_status(expected_status="Running", namespace="sriov-fec-system")
    get_logger().log_info("Delete VRB2 pod")
    kubectl_delete_kw.delete_resources(f"{base_path}/sriov-pod-k8s-manifest-{pf_driver}-{vf_driver}-vrb2-{index}.yaml")
...
def test_fec_operator_pf_driver_igb_uio_vf_drive_igb_uio_vrb2(request: FixtureRequest):
    ...
    Teardown:
        - Delete VRB2 pod and configuration resources
    ...
    def teardown():
        get_logger().log_teardown_step("Delete VRB2 pod and configuration resources")
        delete_fec_pods(ssh_connection)
    ...
    vrb2_devices = get_vrb2_devices_address(ssh_connection)
    validate_not_equals(len(vrb2_devices), 0, "Enabled VRB2 devices found in the system")

    get_logger().log_test_case_step("Generate igb_uio configuration YAML files for each VRB2 device and pod files")
    generate_vrb2_configuration_file(ssh_connection, vrb2_devices, pf_driver, vf_driver, base_path)
    ...
    for index, (host_name, device_address) in enumerate(vrb2_devices):
        ...
        get_logger().log_test_case_step("Apply sriov-fec-operator VRB2 configuration and run accelerator pod and verify if pod is running")
        pod_name = apply_vrb2_configuration_and_pod(ssh_connection, pf_driver, vf_driver, index, base_path, host_name)
        ...
        get_logger().log_test_case_step("Delete VRB2 configuration and pod resources.")
        delete_vrb2_configuration_and_pod(ssh_connection, base_path, pf_driver, vf_driver, index, host_name)
...
@mark.lab_has_vrb2
@mark.lab_has_page_size_1gb
def test_fec_operator_pf_driver_igb_uio_vf_drive_vfio_pci_vrb2(request: FixtureRequest):
    """Verify VRB2 FEC operator with igb_uio pf/vf drivers.
...
@mark.lab_has_vrb2
@mark.lab_has_page_size_1gb
def test_fec_operator_pf_driver_vfio_pci_vf_drive_vfio_pci_vrb2(request: FixtureRequest):
    """Verify VRB2 FEC operator with igb_uio pf/vf drivers.
