...
N3000_DEVICE_ID = "0d8f"
VF_DEVICE_ID = "0d90"
VF_VENDOR_ID = "8086"


def get_n3000_devices_address(ssh_connection: SSHConnection) -> list[tuple[str, str]]:
    """Get N3000 devices address for all hosts in the system.

    Args:
        ssh_connection (SSHConnection): SSH connection to the active controller.

    Returns:
        list[tuple[str, str]]: Tuples of hostname and N3000 device address.
    """
    ...
    for host_name in system_host_list_output.get_host_names():
        ...
        for device in device_output.get_device_address_by_device_id(N3000_DEVICE_ID):
            n3000_devices.append((host_name, device))

    for host_name, address in n3000_devices:
        get_logger().log_info(f"Host: {host_name}, Address: {address}")

    return n3000_devices


def generate_n3000_configuration_file(ssh_connection: SSHConnection, n3000_devices: list[tuple[str, str]], pf_driver: str, vf_driver: str, base_path: str) -> None:
    """Generate N3000 configuration file and pod YAML file from template.

    Args:
        ssh_connection (SSHConnection): SSH connection to the target host.
        n3000_devices (list[tuple[str, str]]): Tuples of hostname and N3000 device address.
    ...
    pod_config_files = []
    for index, (host_name, device_adress) in enumerate(n3000_devices):
        template_path = get_stx_resource_path("resources/cloud_platform/sriov-fec-operator/sriov-fec-operator-n3000.yaml.j2")
        replacement_dict = {
            "name": f"n3000-{index}",
            "hostName": host_name,
            "pfDriver": pf_driver,
        ...
        remote_file = yaml_kw.generate_yaml_file_from_template(template_path, replacement_dict, f"sriov-fec-operator-{pf_driver}-{vf_driver}-n3000-{index}.yaml", base_path, preserve_order=True)
        operator_config_files.append(remote_file)
        get_logger().log_info(f"Generated operator config: {remote_file}")

        template_path = get_stx_resource_path("resources/cloud_platform/sriov-fec-operator/sriov-pod-n3000.yaml.j2")
        replacement_dict = {
            "podName": f"n3000-{index}",
        }
        remote_file = yaml_kw.generate_yaml_file_from_template(template_path, replacement_dict, f"sriov-pod-k8s-manifest-{pf_driver}-{vf_driver}-n3000-{index}.yaml", base_path, preserve_order=True)
...
def apply_n3000_configuration_and_pod(ssh_connection: SSHConnection, pf_driver: str, vf_driver: str, index: int, base_path: str, host_name: str) -> str:
    """Apply sriov-fec-operator N3000 configuration and run accelerator pod and verify if pod is running.

    Args:
        ssh_connection (SSHConnection): SSH connection to the active controller.
        pf_driver (str): PF driver name.
        vf_driver (str): VF driver name.
        index (int): N3000 device index.
    ...
    operator_file = f"{base_path}/sriov-fec-operator-{pf_driver}-{vf_driver}-n3000-{index}.yaml"
    pod_file = f"{base_path}/sriov-pod-k8s-manifest-{pf_driver}-{vf_driver}-n3000-{index}.yaml"

    get_logger().log_info("Apply sriov-fec-operator N3000 card configuration file")
    KubectlFileApplyKeywords(ssh_connection).apply_resource_from_yaml(operator_file)
    get_logger().log_info("Verify if sriov-fec-operator N3000 card configuration file was applied and pod is running")
    ...
    KubectlGetPodsKeywords(ssh_connection).wait_for_pods_to_reach_status(expected_status="Running", namespace="sriov-fec-system")
    get_logger().log_info("Run N3000 pod for accelerator card")
    ...
    get_logger().log_info("Verify N3000 pod is running")
    pod_name = get_pods_kw.get_pods(namespace="default").get_unique_pod_matching_prefix(starts_with="n3000")
    pod_status = get_pods_kw.wait_for_pod_status(pod_name, "Running", "default")
    validate_equals(pod_status, True, "N3000 pod status is running")
...
def delete_n3000_configuration_and_pod(ssh_connection: SSHConnection, base_path: str, pf_driver: str, vf_driver: str, index: int, host_name: str) -> None:
    """Delete N3000 operator configuration and pod resource files.
    ...
    Args:
        ...
        host_name (str): Host Name
    """
    get_logger().log_info("Delete N3000 configuration")
    kubectl_delete_kw = KubectlFileDeleteKeywords(ssh_connection)
    kubectl_delete_kw.delete_resources(f"{base_path}/sriov-fec-operator-{pf_driver}-{vf_driver}-n3000-{index}.yaml")
    get_logger().log_info("Verify if sriov-fec-operator N3000 card configuration file was applied and pod is running")
    KubectlGetSriovFecNodeConfigKeywords(ssh_connection).wait_for_configured_status(
        host_name,
        expected_status="InProgress",
    ...
    KubectlGetSriovFecNodeConfigKeywords(ssh_connection).wait_for_configured_status(host_name)
    KubectlGetPodsKeywords(ssh_connection).wait_for_pods_to_reach_status(expected_status="Running", namespace="sriov-fec-system")
    get_logger().log_info("Delete N3000 pod")
    kubectl_delete_kw.delete_resources(f"{base_path}/sriov-pod-k8s-manifest-{pf_driver}-{vf_driver}-n3000-{index}.yaml")
...
def delete_fec_pods(ssh_connection: SSHConnection) -> None:
    ...
    pod_prefixes = ["vrb2", "acc100", "acc200", "n3000"]
    get_pods_kw = KubectlGetPodsKeywords(ssh_connection)
...
def test_fec_operator_pf_driver_igb_uio_vf_drive_igb_uio_n3000(request: FixtureRequest):
    ...
    Teardown:
        - Delete N3000 pod and configuration resources
    ...
    def teardown():
        get_logger().log_teardown_step("Delete N3000 pod and configuration resources")
        delete_fec_pods(ssh_connection)
    ...
    n3000_devices = get_n3000_devices_address(ssh_connection)
    validate_not_equals(len(n3000_devices), 0, "Enabled N3000 devices found in the system")

    get_logger().log_test_case_step("Generate igb_uio configuration YAML files for each N3000 device and pod files")
    generate_n3000_configuration_file(ssh_connection, n3000_devices, pf_driver, vf_driver, base_path)
    ...
    for index, (host_name, device_address) in enumerate(n3000_devices):
        ...
        get_logger().log_test_case_step("Apply sriov-fec-operator N3000 configuration and run accelerator pod and verify if pod is running")
        pod_name = apply_n3000_configuration_and_pod(ssh_connection, pf_driver, vf_driver, index, base_path, host_name)
        ...
        get_logger().log_test_case_step("Delete N3000 configuration and pod resources.")
        delete_n3000_configuration_and_pod(ssh_connection, base_path, pf_driver, vf_driver, index, host_name)
...
@mark.lab_has_n3000
@mark.lab_has_page_size_1gb
def test_fec_operator_pf_driver_igb_uio_vf_drive_vfio_pci_n3000(request: FixtureRequest):
    """Verify N3000 FEC operator with igb_uio pf/vf drivers.
...
@mark.lab_has_n3000
@mark.lab_has_page_size_1gb
def test_fec_operator_pf_driver_vfio_pci_vf_drive_vfio_pci_n3000(request: FixtureRequest):
    """Verify N3000 FEC operator with igb_uio pf/vf drivers.
