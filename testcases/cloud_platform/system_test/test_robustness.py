...
def test_standby_controller_lock_unlock() -> None:
    ...
    timing_logger = TimingLogger("standby_controller_lock_unlock", column_headers=["Lock Time (s)", "Unlock Time (s)", "Pod Recovery Time (s)"])
...
def _execute_worker_lock_unlock(worker_names: list[str], timing_logger: TimingLogger) -> None:
    ...
    Args:
        worker_names(list[str]): List of worker hostnames to lock/unlock.
        timing_logger(TimingLogger): TimingLogger instance to record metrics.
    """
    ssh_connection = LabConnectionKeywords().get_active_controller_ssh()
    host_keywords = SystemHostLockKeywords(ssh_connection)
    pod_keywords = KubectlGetPodsKeywords(ssh_connection)
    ...
    for worker_name in worker_names:
        validate_equals(host_keywords.wait_for_host_unlocked(worker_name, unlock_wait_timeout=600), True, f"Host {worker_name} should be unlocked after unlock operation")
...
def _execute_worker_reboot(worker_names: list[str], timing_logger: TimingLogger) -> None:
    ...
    Args:
        worker_names(list[str]): List of worker hostnames to reboot.
        timing_logger(TimingLogger): TimingLogger instance to record metrics.
    """
    ssh_connection = LabConnectionKeywords().get_active_controller_ssh()
    ...
    get_logger().log_test_case_step("Waiting for hosts to be unlocked (available and enabled)...")
    for worker_name in worker_names:
        validate_equals(host_lock_keywords.wait_for_host_unlocked(worker_name, unlock_wait_timeout=600), True, f"Host {worker_name} should be unlocked after reboot")
...
def test_lock_unlock_multiple_workers() -> None:
    ...
    timing_logger = TimingLogger("worker_lock_unlock", column_headers=["Pod Migration Time (s)", "Hosts Available Time (s)", "Hosts Enabled Time (s)", "Pod Recovery Time (s)"])
...
def test_reboot_multiple_workers() -> None:
    ...
    SetupStressPods(ssh_connection).setup_stress_pods(benchmark="mixed")
    timing_logger = TimingLogger("worker_reboot", column_headers=["Pod Migration Time (s)", "Hosts Available Time (s)", "Hosts Enabled Time (s)", "Pod Recovery Time (s)"])
...
def test_lock_unlock_single_workers() -> None:
    ...
    SetupStressPods(ssh_connection).setup_stress_pods(benchmark="mixed")
    timing_logger = TimingLogger("worker_lock_unlock_single", column_headers=["Pod Migration Time (s)", "Host Available Time (s)", "Host Enabled Time (s)", "Pod Recovery Time (s)"])
    _execute_worker_lock_unlock(worker_names, timing_logger)
...
def test_reboot_single_worker() -> None:
    ...
    SetupStressPods(ssh_connection).setup_stress_pods(benchmark="mixed")
    timing_logger = TimingLogger("worker_reboot_single", column_headers=["Pod Migration Time (s)", "Host Available Time (s)", "Host Enabled Time (s)", "Pod Recovery Time (s)"])
    _execute_worker_reboot(worker_names, timing_logger)
...
def test_reboot_simplex() -> None:
    ...
    host_lock_keywords = SystemHostLockKeywords(ssh_connection)
    pod_keywords = KubectlGetPodsKeywords(ssh_connection)
    timing_logger = TimingLogger("active_controller_reboot_simplex", column_headers=["Hosts Available Time (s)", "Hosts Enabled Time (s)", "Pod Recovery Time (s)"])
    log_extractor = KpiExtractor(ssh_connection)
    ...
    validate_equals(host_lock_keywords.wait_for_host_unlocked(active_name, unlock_wait_timeout=900), True, f"Host {active_name} should be unlocked after reboot")

    # Pods recovery time calc
    recovery_start = time.time()
    get_logger().log_test_case_step("Waiting for all pods to be running...")
    pod_keywords.wait_for_all_pods_status(["Running", "Succeeded", "Completed"], timeout=600)
    pod_recovery_time = time.time() - recovery_start
    ...
    host_available_time = host_enabled_time - log_extractor.extract_dor_recovery_timing(active_name)
