...
@mark.p0
def test_deploy_small_benchmark(request: pytest.FixtureRequest):
    """
    Deploys pods for the small benchmark type.
...
@mark.p0
def test_deploy_benchmark_pods_mixed(request: pytest.FixtureRequest):
    """
...
@mark.p0
def test_deploy_benchmark_pods_large(request: pytest.FixtureRequest):
    """
    Deploys pods for the large benchmark type.
    Scale up and down the deployments and measures the time taken for each operation.
...
def deploy_benchmark_pods(request: pytest.FixtureRequest, benchmark: str):
    """
    ...
    Args:
        request (pytest.FixtureRequest): pytest request object
        benchmark (str): The type of benchmark to run (mixed, large, small, stress)
    """
...
@mark.p0
def test_simultaneous_activities(request: pytest.FixtureRequest):
    """
    ...
    timing_logger = TimingLogger("simultaneous_activities", column_headers=["Initial Scale Up Time (s)", "Simultaneous Scale Time (s)"])

    start_time = datetime.utcnow()
    stress_pods = SetupStressPods(ssh_connection)

    get_logger().log_test_case_step(f"Setting up and scaling primary namespace {prime_ns}")
    deploy_time_group1, initial_scale_time__group1 = stress_pods.setup_stress_pods(benchmark=benchmark, namespace=prime_ns)

    get_logger().log_test_case_step(f"Setting up secondary namespace {second_ns}")
    stress_pods.deploy_stress_pods(benchmark=benchmark, namespace=second_ns)
    ...
    validate_equals(pod_getter.wait_for_all_pods_status(expected_statuses=["Running", "Completed"]), True, "All pods are healthy")
...
@mark.p0
def test_crash_rollout_restart(request: pytest.FixtureRequest):
    """
...
@mark.p0
def test_pod_scaling_background(request: pytest.FixtureRequest):
    """
