...
def test_add_parameter() -> None:
    ...
    param_list_output = service_keywords.get_system_service_parameter_list_by_section("sysctl")
    
    for param_name, param_value in SYSCTL_PARAMS:
        get_logger().log_info(f"Adding sysctl parameter {param_name}={param_value}")
        
        param_found = param_list_output.get_parameter_by_name(param_name)
        if param_found:
            ...
            get_logger().log_info(f"Adding new parameter")
...
def test_verify_persistence() -> None:
    ...
    for param_name, param_value in SYSCTL_PARAMS:
        ...
        validate_equals(param_found.get_value(), param_value, f"Parameter {param_name} value in database")
